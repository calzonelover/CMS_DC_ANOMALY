...
class SparseContractiveAutoencoder(BaseModel):
    def __init__(self, LAMBDA=1e-4, input_dim=[2806], batch_size=1024, learning_rate=1e-4, beta1=0.7, beta2=0.9,**kwargs):
        super(SparseContractiveAutoencoder, self).__init__(**kwargs)
        # parameters
        self.LAMBDA = LAMBDA
        self.batch_size = batch_size
        self.learning_rate = learning_rate
        with self.graph.as_default():
            ...
            with tf.name_scope("Loss"):
                ...
                with tf.name_scope("contractive"):
                    ...
                    tf.summary.scalar("Loss_contractive", self.loss_con)
                self.total_loss = tf.add_n([self.loss_mse, self.loss_con, self.loss_reg], name="total_loss")
                tf.summary.scalar("Total_loss", self.total_loss)
            # optimizer
            with tf.name_scope("Optimizer"):
                self.optimizer = tf.train.AdamOptimizer(
...
class SparseVariationalAutoencoder(BaseModel):
    def __init__(self, LAMBDA=1e-4, input_dim=[2806], batch_size=1024, learning_rate=1e-4, beta1=0.7, beta2=0.9,**kwargs):
        super(SparseVariationalAutoencoder, self).__init__(**kwargs)
        # parameters
        self.LAMBDA = LAMBDA
        self.batch_size = batch_size
        self.learning_rate = learning_rate
        with self.graph.as_default():
            ...
            with tf.name_scope("Loss"):
                ...
                with tf.name_scope("l1_regularization"):
                    self.loss_reg = self.LAMBDA * self.sparse_loss([self.W1, self.W2, self.W3, self.W4, self.W5, self.W6])
                    tf.summary.scalar("Loss_reg", self.loss_reg)
                self.total_loss = tf.math.add_n([self.loss_mse, self.loss_kl, self.loss_reg], name="total_loss")
                tf.summary.scalar("Total_loss", self.total_loss)
            # optimizer
            with tf.name_scope("Optimizer"):
                self.optimizer = tf.train.AdamOptimizer(
...
class ContractiveVariationalAutoencoder(BaseModel):
    def __init__(self, LAMBDA=1e-4, input_dim=[2806], batch_size=1024, learning_rate=1e-4, beta1=0.7, beta2=0.9,**kwargs):
        super(ContractiveVariationalAutoencoder, self).__init__(**kwargs)
        # parameters
        self.LAMBDA = LAMBDA
        self.batch_size = batch_size
        self.learning_rate = learning_rate
        with self.graph.as_default():
            ...
            with tf.name_scope("model_body"):
                ...
                self.W3 = self.weight_variable([64, 32])
                self.wxb3 = self.fc_layer(self.encoder2, self.W3, 32)
                self.encoding = self.PReLU(self.wxb3, name="encoding")
                ...
                self.W4 = self.weight_variable([16, 64])
                self.wxb4 = self.fc_layer(self.sampling, self.W4, 64)
                self.decoder1 = self.PReLU(self.wxb4, name="decoder1")
            ...
            with tf.name_scope("Loss"):
                ...
                with tf.name_scope("contractive"):
                    self.loss_con = self.LAMBDA * tf.add_n([
                            self.contractive_loss_prelu("model_body", "encoder1", self.W1, self.wxb1),
                            self.contractive_loss_prelu("model_body", "encoder2", self.W2, self.wxb2),
                            self.contractive_loss_prelu("model_body", "encoding", self.W3, self.wxb3),
                            self.contractive_loss_prelu("model_body", "decoder1", self.W4, self.wxb4),
                            self.contractive_loss_prelu("model_body", "decoder2", self.W5, self.wxb5),
                            self.contractive_loss_sigmoid(self.W6, self.decoder3),
                        ])
                    tf.summary.scalar("Loss_contractive", self.loss_con)
                self.total_loss = tf.add_n([self.loss_mse, self.loss_con, self.loss_kl], name="total_loss")
                tf.summary.scalar("Total_loss", self.total_loss)
            # optimizer
            with tf.name_scope("Optimizer"):
                self.optimizer = tf.train.AdamOptimizer(
...
class StandardAutoencoder(BaseModel):
    def __init__(self, LAMBDA=1e-4, input_dim=[2806], batch_size=1024, learning_rate=1e-4, beta1=0.7, beta2=0.9,**kwargs):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope("model_body"):
                ...
                self.W3 = self.weight_variable([64, 32])
                self.wxb3 = self.fc_layer(self.encoder2, self.W3, 32)
                self.encoding = self.PReLU(self.wxb3, name="encoder3")
            ...
            with tf.name_scope("Loss"):
                ...
                with tf.name_scope("KL_divergence"):
                    self.loss_kl = self.kl_divergence(self.means, self.sigmas)
                    tf.summary.scalar("Loss_KL", self.loss_kl)
                self.total_loss = tf.add_n([self.loss_mse, self.loss_con, self.loss_reg, self.loss_kl], name="total_loss")
                tf.summary.scalar("Total_loss", self.total_loss)
            # optimizer
            with tf.name_scope("Optimizer"):
                self.optimizer = tf.train.AdamOptimizer(
